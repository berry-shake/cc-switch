    ...
    match app_type {
        ...
        AppType::Pi | AppType::Omp => {
            return Err(AppError::InvalidInput(format!(
                "{} providers use the native provider service",
                app_type.as_str()
            )));
        }
    }
...
pub fn sync_current_to_live(state: &AppState) -> Result<(), AppError> {
    ...
    for app_type in AppType::all() {
        if matches!(app_type, AppType::Pi | AppType::Omp | AppType::Mcode) {
            continue;
        }
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        ...
        AppType::Pi | AppType::Omp => Err(AppError::InvalidInput(format!(
            "{} providers are read from the native models file",
            app_type.as_str()
        ))),
