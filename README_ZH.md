...
### Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes、MiniMax Code、Pi 和 Oh My Pi（OMP）的全方位管理工具
...
现代 AI 编程依赖于 Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes、MiniMax Code、Pi 和 Oh My Pi（OMP）等工具——但每个工具都有自己的配置格式。切换 API 供应商意味着手动编辑 JSON、YAML、TOML 或 `.env` 文件，而在多个工具之间缺乏一个统一管理 MCP、Skills 的方式。
...
- **一个应用，十一个工具** — 在单一界面中管理 Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes、MiniMax Code、Pi 和 OMP
- **告别手动编辑** — 50+ 供应商预设，包括 AWS Bedrock、NVIDIA NIM 和社区中转服务；一键即可切换
- **统一 MCP、Skills 管理** — 统一管理 Claude、Codex、Gemini、Grok Build、OpenCode、Hermes、MiniMax Code 和 OMP 的 MCP 服务，并在支持的应用间双向同步 Skills
...
- **11 个支持工具，50+ 预设** — Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes、MiniMax Code、Pi 和 OMP；复制 key 即可一键导入
...
- **统一 MCP 面板** — 管理 Claude、Codex、Gemini、Grok Build、OpenCode、Hermes、MiniMax Code 和 OMP 的 MCP 服务器，双向同步，支持 Deep Link 导入
- **Prompts** — Markdown 编辑器，跨应用同步（CLAUDE.md / AGENTS.md / SYSTEM.md / APPEND_SYSTEM.md / GEMINI.md），回填保护
- **Skills** — 从 GitHub 仓库或 ZIP 文件一键安装，自定义仓库管理，Pi/OMP 独立部署，支持软连接和文件复制
...
CC Switch 支持十一个工具：**Claude Code**、**Claude Desktop**、**Codex**、**Gemini CLI**、**Grok Build**、**OpenCode**、**OpenClaw**、**Hermes**、**MiniMax Code**、**Pi** 和 **Oh My Pi（OMP）**。每个工具都有专属的供应商与配置管理。
