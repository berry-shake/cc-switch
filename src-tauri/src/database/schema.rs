...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    18 => {
                        log::info!("迁移数据库从 v18 到 v19（Skills/MCP 添加 OMP 支持）");
                        Self::migrate_v18_to_v19(conn)?;
                        Self::set_user_version(conn, 19)?;
                    }
                    19 => {
                        // Fork v19 added OMP; upstream v19 added MiniMax Code.
                        // Reconcile both layouts without overwriting existing flags.
                        log::info!(
                            "迁移数据库从 v19 到 v20（统一 OMP/MiniMax Code 的 Skills/MCP 列）"
                        );
                        Self::migrate_v18_to_v19(conn)?;
    ...
    /// v18 -> v19: independent OMP enablement flags for Skills and MCP.
    ...
    fn repair_current_model_pricing(conn: &Connection) -> Result<(), AppError> {
        let pricing_fixes = [
            ...
            // 2026-08-22 GPT-5.6 Sol 官方促销价：5/30/0.50/6.25 → 4/20/0.40/5。
            // 主模型、裸别名与 effort 别名必须同步；四个旧价字段完全命中时
            // 才更新，任一字段被用户自定义过都保持不动。本组位于早期 cache write
            // 0 → 6.25 修复之后，因此从旧版直升的 Sol 主模型也能连续收敛到现价。
            (
                "gpt-5.6-sol",
                "GPT-5.6 Sol",
                "4",
...
mod tests {
    ...
    #[test]
    fn migrate_v18_to_v19_adds_independent_omp_flags() -> Result<(), AppError> {
        let conn = Connection::open_in_memory()?;
        conn.execute_batch(
            "CREATE TABLE mcp_servers (
                id TEXT PRIMARY KEY,
        ...
        assert_eq!(Database::get_user_version(&conn)?, SCHEMA_VERSION);
        let mcp_flags: (i64, i64) = conn.query_row(
            "SELECT enabled_hermes, enabled_omp FROM mcp_servers WHERE id = 'mcp-1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
        let skill_flags: (i64, i64) = conn.query_row(
            "SELECT enabled_hermes, enabled_omp FROM skills WHERE id = 'skill-1'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )?;
    ...
    fn assert_v19_app_flags_migrate(
        ...
        for table in ["mcp_servers", "skills"] {
            conn.execute_batch(&format!(
                "CREATE TABLE {table} (id TEXT PRIMARY KEY, enabled_hermes BOOLEAN NOT NULL DEFAULT 0, payload TEXT);
                 INSERT INTO {table} VALUES ('existing', 1, 'keep existing metadata');"
            ))?;
            for (column, present) in [
                ("enabled_omp", omp_present),
                ("enabled_mcode", mcode_present),
            ] {
                if present {
                    conn.execute_batch(&format!(
        ...
        conn.execute_batch(
            "CREATE TABLE session_log_sync (file_path TEXT PRIMARY KEY, last_line_offset INTEGER, last_byte_offset INTEGER);
             INSERT INTO session_log_sync VALUES ('existing-rollout', 42, 12345);",
        )?;
        Database::set_user_version(&conn, 19)?;

        // Repeating startup must preserve both existing flags and newly defaulted flags.
        for _ in 0..2 {
            Database::apply_schema_migrations_on_conn(&conn)?;
            assert_eq!(Database::get_user_version(&conn)?, SCHEMA_VERSION);
            for table in ["mcp_servers", "skills"] {
                let values: (bool, bool, bool, String) = conn.query_row(
                    &format!("SELECT enabled_hermes, enabled_omp, enabled_mcode, payload FROM {table} WHERE id = 'existing'"),
                    [],
                ...
                assert_eq!(
                    ...
                    (
                        ...
                        omp_present,
                        mcode_present,
                        "keep existing metadata".to_string()
                    )
                );
            }
            let cursor: (i64, i64) = conn.query_row(
                "SELECT last_line_offset, last_byte_offset FROM session_log_sync WHERE file_path = 'existing-rollout'",
                [],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )?;
            assert_eq!(cursor, (42, 12345));
        }
        Ok(())
    }

    #[test]
    fn migrate_fork_v19_preserves_omp_and_adds_mcode() -> Result<(), AppError> {
        assert_v19_app_flags_migrate(true, false)
    }

    #[test]
    fn migrate_upstream_v19_preserves_mcode_and_adds_omp() -> Result<(), AppError> {
        assert_v19_app_flags_migrate(false, true)
    }

    #[test]
    fn migrate_v19_with_both_flags_preserves_each() -> Result<(), AppError> {
        assert_v19_app_flags_migrate(true, true)
    }
}
