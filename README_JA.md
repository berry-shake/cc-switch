...
### Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes、Pi、Oh My Pi（OMP）のオールインワン管理ツール
...
最新の AI コーディングは Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes、Pi、Oh My Pi（OMP）などのツールに依存していますが、各ツールの設定形式はバラバラです。API プロバイダを切り替えるたびに JSON、YAML、TOML、`.env` ファイルを手動で編集する必要があり、複数ツール間で MCP や Skills を統一的に管理する手段もありません。
...
- **1 つのアプリで 10 のツール** -- Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes、Pi、OMP を単一インターフェースで管理
- **手動編集は不要** -- AWS Bedrock、NVIDIA NIM、コミュニティリレーなど 50 以上のプロバイダプリセットを内蔵。選んで切り替えるだけ
- **統一 MCP・Skills 管理** -- Claude、Codex、Gemini、Grok Build、OpenCode、Hermes、OMP の MCP サーバーと、対応アプリの Skills を双方向同期で管理
...
- **10 の対応ツール、50 以上のプリセット** -- Claude Code、Claude Desktop、Codex、Gemini CLI、Grok Build、OpenCode、OpenClaw、Hermes、Pi、OMP。キーをコピーしてワンクリックでインポート
...
- **統一 MCP パネル** -- Claude、Codex、Gemini、Grok Build、OpenCode、Hermes、OMP の MCP サーバーを管理、双方向同期、Deep Link インポート対応
...
CC Switch は **Claude Code**、**Claude Desktop**、**Codex**、**Gemini CLI**、**Grok Build**、**OpenCode**、**OpenClaw**、**Hermes**、**Pi**、**Oh My Pi（OMP）** の 10 のツールに対応しています。各ツールに専用のプロバイダ管理と設定管理が用意されています。
